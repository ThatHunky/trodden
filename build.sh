#!/bin/bash
# Builds Trodden: JDK 25 emitting Java 21 bytecode (--release 21, so the jar also loads on 1.21.x
# servers that run Java 21), against paper-api 26.2 from the Matsuri server's libraries/, plus HuskClaims
...
rm -rf "$HERE/build"
rm -f "$HERE"/Trodden-*.jar
mkdir -p "$HERE/build/classes" "$HERE/build/test"
cp "$HERE/plugin.yml" "$HERE/config.yml" "$HERE/build/classes/"
cp -r "$HERE/lang" "$HERE/build/classes/"   # before TestMain: it checks the lang files from the classpath
"$JDK/javac" --release 21 -encoding UTF-8 -cp "$CP" -d "$HERE/build/classes" \
    $(find "$HERE/src/main/java" -name '*.java')
"$JDK/javac" --release 21 -encoding UTF-8 -cp "$HERE/build/classes:$CP" -d "$HERE/build/test" \
    $(find "$HERE/src/test/java" -name '*.java')
"$JDK/java" -cp "$HERE/build/classes:$HERE/build/test:$CP" dev.thathunky.trodden.TestMain

"$JDK/jar" --create --file "$HERE/Trodden-$VERSION.jar" -C "$HERE/build/classes" .
echo "built: $HERE/Trodden-$VERSION.jar"
